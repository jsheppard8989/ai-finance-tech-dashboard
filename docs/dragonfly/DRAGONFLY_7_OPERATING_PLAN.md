...
Hypothetical fills in the paper book use the trigger plus 5 basis points of adverse slippage, labeled `fill_type: hypothetical`, with commissions recorded even when they are zero. On the canonical trigger of $84.50 that paper fill is $84.54, which is inside the $84.63 max fill. A buy fill above `max_fill` invalidates the card. Live fills later replace the hypothesis. R is recomputed from the real fill and the original stop.
