...
1. **tickers_mentioned format error** — Insight #11 had JSON as string instead of array
   - Fixed: Changed `"[\"MSFT\", \"PLTR\"...]"` to `["MSFT", "PLTR"...]`
2. **Too many insights on main page** — Had 6, should be 5
   - Fixed: Archived "Bitcoin as Hard Asset" (Feb 3) — now exactly 5 insights
3. **Price data key mismatch** — `price_data.json` uses "BTC-USD", code looked for "BTC"
   - Already fixed in earlier commit with fallback logic
4. **Second Order F-X section** — Only shows tickers ranked 11-20, but we only have 11 total
   - This is expected behavior — will populate as more tickers are added

**Current website status:** ✅ All critical issues resolved
