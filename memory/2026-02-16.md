...
### Chart Update Process Fixed
- **Problem:** Candlestick charts were stale (last updated Feb 14) - cron job wasn't pushing to GitHub
- **Solution:**
  - Updated 9am cron job to include `git add site/charts/ && git push`
  - Charts now regenerate daily with `python3 generate_charts.py`
  - Generates 2-week candlestick charts for all tickers
- **Cron job updated:** `Midday Data Refresh (9am)` now pushes charts to GitHub
- **Manual run:** `cd ~/.openclaw/workspace/pipeline && python3 generate_charts.py`
